#
#  Convert udb into design structure matrix
...
foreach my $ref (@references) {
  next if($languageFilter && ($languageFilter ne $ref->file->language));
  my $dest_ref = get_ref($ref->ent); # the entity being referenced
  next unless $dest_ref;
  next if($languageFilter && ($languageFilter ne $dest_ref->file->language));
...
sub depend {
  ...
  my %depend = (
    ...
    'scope' => $ref->scope->simplename,
    'ent_name' => $ref->ent->simplename,
  );
  return \%depend;
}
